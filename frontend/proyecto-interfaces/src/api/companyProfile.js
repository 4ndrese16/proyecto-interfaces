...
const defaultBase = (API_ROOT ? API_ROOT.replace(/\/$/, '') : '') + '/company-profile';
...
export async function getCompanyProfile(apiBase = defaultBase) {
  const client = createClient(apiBase);
  const res = await client.get('/');
  return res.data;
}

export async function saveCompanyProfile(payload, apiBase = defaultBase) {
  const client = createClient(apiBase);
